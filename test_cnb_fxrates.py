...
def test_currency_only():
    ...
    a decimal digit.
    """
    proc = subprocess.run([prog_path, "EUR"], capture_output=True)
...
def test_currency_and_date():
    ...
    """
    proc = subprocess.run([prog_path, "USD", "2022-02-01"], capture_output=True)
    assert proc.returncode == 0
...
def test_invalid_currency_code(code):
    ...
    """
    proc = subprocess.run([prog_path, code], capture_output=True)
    assert proc.returncode == 1
    assert "error" in proc.stderr.decode("utf-8").lower()
...
def test_flipped_currency_and_date():
    ...
    prog must return non-zero exit status and print an error msg into STDERR.
    """
    proc = subprocess.run([prog_path, "2022-02-01", "USD"], capture_output=True)
    assert proc.returncode == 1
    assert "error" in proc.stderr.decode("utf-8").lower()
